...
export const FileUpload: FC<FileUploadProps> = ({ file, handleFileUpload, reset }) => {
    ...
    const handleDrop = (e: DragEvent<HTMLDivElement>) => {
        ...
        if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
            handleFileUpload({ target: { files: e.dataTransfer.files } } as ChangeEvent<HTMLInputElement>);
        }
    };

    return (
        <div
            className={`border-2 border-[#CEC4EB] border-dashed rounded-[12px] h-[240px] flex flex-col items-center justify-center gap-[20px] ${isDragging ? "border-[#6947BF] bg-[#f4f0fa]" : "border-[#CEC4EB] bg-transparent"}`}
            ...
            <div className="flex flex-col items-center justify-center">
                {file ? (
                    ...
                    <>
                        ...
                        <div className={`text-[#7A8196] font-bold text-base ${isDragging ? "text-[#6947BF]" : ""}`}>
                            {isDragging ? "Release to upload your PDF" : "Drag and drop a PDF"}
                        </div>
